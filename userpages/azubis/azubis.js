const YAML_CONFIG_PATH = "azubis.yml";
const ROOT_ID = "azubis-root";

async function loadYamlConfig() {
  const res = await fetch(YAML_CONFIG_PATH, { cache: "no-store" });
  if (!res.ok) {
    throw new Error("Konnte azubis.yml nicht laden: " + res.status);
  }
...
function createMemberCard(entry) {
  ...
  avatar.alt = (entry.name || "Azubi") + " Profilbild";
  ...
  role.textContent = entry.role || "Azubi";
...
initPage().catch((err) => console.error("[azubis.js] Fehler:", err));
